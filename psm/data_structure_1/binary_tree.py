...
class BinaryTree:
    ...
    # 오른쪽 서브 트리반환
    def get_rith_sub_tree(self, cur):
        return cur.right
    ...
    # 오른쪽 서브 트리 만들기
    def make_rifht_sub_tree(self, cur, right):
        cur.right = right


    # 전위 순회로 트리를 순회
    # 인자(cru: 방문 노드, func: 데이터 처리 함수)
    ...
    def postorder_traverse(self, cur, func):
        ...
        # 왼쪽 서브 트리와 오른쪽 서브 트리를 모두 순회한후
        # 마지막으로  방문 노드의 데이터를 인자롤 함수 호출
        func(cur.data)
...
if __name__ == '__main__':
    ...
    # 루트 노드의 왼쪼 ㄱ자식 노드로 노드 2 설정
    bt.make_left_sub_tree(n1, n2)

    # 로투 노드의 오른쪽 자식 노드로 노드 3설정
    bt.make_rifht_sub_tree(n1, n3)

    bt.make_left_sub_tree(n2, n4)
    bt.make_rifht_sub_tree(n2, n5)

    bt.make_left_sub_tree(n3, n6)
    bt.make_rifht_sub_tree(n3, n7)

    # 방문 노드의 데이터를 출력하는 람다 함수
    f = lambda a: print(a, end=' ')

    # 전위 순회
    # 기대 출력값: 1 2 4 5 3 6 7
